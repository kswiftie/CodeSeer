...
class Solution:
    def isPossible(self, vertex_count, edge_list):
        ...
        odd_nodes = [idx for idx in range(vertex_count)
                     if len(graph[idx]) % 2 != 0]
        ...
        if len(odd_nodes) == 2:
            ...
            return any(are_not_adjacent(node_a, mid)
                       and are_not_adjacent(node_b, mid)
                       for mid in range(vertex_count))

        elif len(odd_nodes) == 4:
            a, b, c, d = odd_nodes
            # Reordered condition checks
            return (are_not_adjacent(a, c) and are_not_adjacent(b, d)) or \
                (are_not_adjacent(a, d) and are_not_adjacent(b, c)) or \
                (are_not_adjacent(a, b) and are_not_adjacent(c, d))
