...
class Solution:
    ...
    def _n_sum_helper(
        ...
        if depth == 2:
            self._two_pointer_search(
                sorted_arr, remaining_target, current_combination, result_list
            )
        else:
            ...
            while i < len(sorted_arr) - depth + 1:
                ...
                if i == 0 or (i > 0 and sorted_arr[i] != sorted_arr[i - 1]):
                    ...
                    self._n_sum_helper(
                        ...
                        new_target,
                        depth - 1,
                        current_combination + [current_val],
                        result_list,
                    )
                    dummy_counter += 1  # No functional impact
                i += 1

    def _two_pointer_search(
        self,
        nums: List[int],
        target: int,
